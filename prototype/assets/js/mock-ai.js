const VinTravelAI = {
  ...
  parseUserInput(text) {
    ...
    if (t.includes('một mình') || t.includes('solo')) updates.companions = 'Đi một mình';
  ...
  getNextQuestion(state) {
    ...
    if (!state.companions) {
      return { text: 'Bạn đi cùng ai?', chips: ['2 người (cặp đôi)', 'Gia đình có trẻ nhỏ', 'Nhóm bạn'] };
    }
    if (!state.budget) {
      return {
  ...
  acknowledgment(field, value) {
    const msgs = {
      ...
      companions: 'Đã ghi nhận — mình sẽ ưu tiên địa điểm phù hợp với nhóm của bạn.',
